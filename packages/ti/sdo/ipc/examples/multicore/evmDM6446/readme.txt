...
sure that the C64P and Arm9 targets are not commented out and run "xdc" in this
directory (e.g. packages\ti\sdo\ipc\examples\multicore\evmDM6446). 
Build.targets = [
                    //C28_large,
                    //C64,
                    C64P,
...
1. Load and run the arm image on arm first.
   image: ti_sdo_ipc_examples_platforms_evmDM6446_arm\debug\notify_arm.x9
   
2. After the arm image has reached main(), load and run the dsp image
   on the dsp:
   image: ti_sdo_ipc_examples_platforms_evmDM6446_dsp\debug\notify_dsp.x64P
